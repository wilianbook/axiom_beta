...
def jtag_sseq(i2c, addr, data, mod, val):
    norm, last = addr[0], addr[1]
    res = []
...
def jtag_tds(i2c, bits, exit=True):
    data, mod, val = bit_split(leba(bits))
    last = 0x14 if exit else 0x18
    data, mod, val = jtag_sseq(i2c, [0x18, last], data, mod, val)
...
class JTag:
    ...
    def shift(self, bits, exit=True):
        ...
        bits = jtag_tds(self.i2c, bits, exit)
        if exit:
            self.transition("1")
